...
/**
 * เนื้อหาหน้าการพัฒนาตนเอง — ใช้ร่วมกันระหว่าง /development และ /development/[year]
 * รับปีเป็นตัวเลขแทนการอ่านจาก ?year= เพื่อให้หน้าถูกแคชได้
 */
export default async function DevelopmentView({ year }: { year?: number }) {
  const years = await getSelfDevYears()
  const cur = year || years[0] || 0
    ...
    <>
      ...
      <div className="max-w-[1240px] mx-auto px-4 md:px-10 -mt-8 relative">
        <div className="card-soft p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <YearTabs years={years} current={cur} hrefFor={(y) => `/development/${y}`} />
        </div>
      </div>
